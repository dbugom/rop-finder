...
Same prerequisites as building from source: rustup and a C compiler. The
package names carry the product prefix because `rf-cli` and `rf-core` on
crates.io are unrelated crates owned by other people; `docs/PUBLISHING.md`
has the full package/library name map.

**Not available until the 1.0.0 release is uploaded.** It is packaged and
`cargo publish --dry-run`-verified, and nothing has been pushed to crates.io
yet; until it is, use the source build below.
...
`--cfg-aware` buys you nothing here, and the tool now says so instead of silently emptying the list. Windows kernel images of the CFG generation carry `IMAGE_DLLCHARACTERISTICS_GUARD_CF` — software Control Flow Guard, a forward-edge mitigation — with no Intel CET landing pads anywhere in the file. Since v0.2 (`CRIT-01`) the flag is table-aware, so it no longer wipes the ROP gadgets out, and it prints `--cfg-aware: this binary contains no endbr32/endbr64 landing pads … the flag constrains nothing` when there is nothing for it to constrain. Check `cet_compat` in `--info` rather than assuming GUARD_CF implies CET.
...
<!-- Generated by `cargo test -p rop-finder-mcp --test manual_schema`. Do not edit by hand: the test regenerates it from the server's own tools/list and outputSchema and fails if this block has drifted. -->
...
The filter is *table-aware*, which is what `CRIT-01` fixed in v0.2. IBT constrains **indirect** transfers, so a JOP or SYS gadget — one you reach through `jmp`/`call` on a register — only survives if it begins with an `endbr64`/`endbr32` landing pad. A ROP gadget is reached by `ret`, which IBT does not police, so ROP gadgets survive regardless. Before that fix the flag demanded a landing pad on every gadget and returned zero on everything.

**Two caveats you need before you rely on this.**

First, **there is still no CET-marked binary in this repository's fixture corpus**: none of the 24 fixtures contains an `endbr64` (`f3 0f 1e fa`) or `endbr32` (`f3 0f 1e fb`) byte sequence anywhere. So the JOP/SYS half of this filter has never been exercised against a real hardened binary — only against synthetic unit tests. What the flag *does* tell you is when it is doing nothing: on a binary with no landing pads it prints

```
[Warning] --cfg-aware: this binary contains no endbr32/endbr64 landing pads, so
Intel CET/IBT is not enforced on it and the flag constrains nothing. (PE GUARD_CF
is a different mitigation and is not what --cfg-aware models.)
```

which is `CRIT-01`'s promised scan-time warning and fires on every fixture here. Measured on this corpus, `--cfg-aware` returns a non-zero gadget count on 21 of the 24 fixtures (for example 8,389 on `elf-Linux-x64` and 2,097 on `pe-x64-cmd-v6.1.7601`) — every one of them ROP-table gadgets, with the warning attached.

Second, GUARD_CF is **not** what this flag models. GUARD_CF is Microsoft's software Control Flow Guard — a runtime bitmap check on indirect call targets, forward-edge only — and is a different mitigation from Intel CET/IBT. `--info` reports the two separately, from separate evidence: `guard_cf` from `IMAGE_DLLCHARACTERISTICS_GUARD_CF` in `DllCharacteristics`, and `cet_compat` from the `IMAGE_DEBUG_TYPE_EX_DLLCHARACTERISTICS` (type 20) debug-directory record. Read `cet_compat` before reaching for `--cfg-aware`; a GUARD_CF-only binary is not a CET binary.
